package com.mobile.zxw.myapplication.ui.widget;
...
//    private static final String TAG = "XListView";
    ...
    private boolean isComplete = false;  //是否所有数据加载完成
    ...
    public void setPullLoadEnable(boolean enable ) {
        ...
        if (!mEnablePullLoad) {
            ...
            if(!isComplete){
                ...
                mFooterView.setPadding(0, 0, 0, 0);
                mFooterView.show();
                mFooterView.setState(XFooterView.STATE_COMPLETE);
     ...
     * @param enable 是否显示底部
     */
    public void setPullLoadComplete(boolean enable ) {
        isComplete = enable;
    }
    ...
    private void updateFooterHeight(float delta) {
        ...
        if (mEnablePullLoad && !mPullLoading && !isComplete) {
            System.out.println("----------------height"+height);
            System.out.println("----------------PULL_LOAD_MORE_DELTA"+PULL_LOAD_MORE_DELTA);
            ...
            default:
                ...
                if (getFirstVisiblePosition() == 0) {
                    ...
                    if (mEnablePullLoad && mFooterView.getBottomMargin() > PULL_LOAD_MORE_DELTA) {
                        System.out.println("----------------PULL_LOAD_MORE_DELTA"+isComplete);
                        if(!isComplete){
                            startLoadMore();
                        }
                    }
    ...
    public void onScrollStateChanged(AbsListView view, int scrollState) {
        ...
        if (scrollState == OnScrollListener.SCROLL_STATE_IDLE) {
            if (mEnableAutoLoad && getLastVisiblePosition() == getCount() - 1) {
                System.out.println("----------------SCROLL_STATE_IDLE"+isComplete);
                if(!isComplete){
                    startLoadMore();
                }
            }
