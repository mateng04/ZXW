package com.mobile.zxw.myapplication.bean;

import java.util.List;

public class RecruitDetailsBean {

//    title（标题） updateTime（更新时间）browsing（浏览次数）
//    zwmc（职位名称） xbyq(性别要求) yxqk(月薪情况) gzjy(工作经验) nlfw(年龄范围) zdxl(最低学历)
//    mzyq(民族要求) zprs(招聘人数)
//    yxqz(有效期至) gzdd(工作地点) zwms(职位描述) tupian(图片集合) lxdh(联系电话) lxry(联系人员) id(唯一标示)
//    isshowphone(是否隐藏手机号码) shoufei(查看手机号每次费用)

    private String recruitTile;
    private String updateTime;
    private String browsing;
    private String zwmc;
    private String xbyq;
    private String yxqk;
    private String xinzidanwei;
    private String gzjy;
    private String nlfw;
    private String zdxl;
    private String mzyq;
    private String zprs;
    private String yxqz;
    private String gzdd;
    private String zwms;
    private String lxdh;
    private String lxry;
    private String gzsj;
    private String id;
    private String isshowphone;
    private String shoufei;
    private List<String> tupian;
    private String kxsj;

    public String getKxsj() {
        return kxsj;
    }

    public void setKxsj(String kxsj) {
        this.kxsj = kxsj;
    }

    public String getRecruitTile() {
        return recruitTile;
    }

    public void setRecruitTile(String recruitTile) {
        this.recruitTile = recruitTile;
    }

    public String getUpdateTime() {
        return updateTime;
    }

    public void setUpdateTime(String updateTime) {
        this.updateTime = updateTime;
    }

    public String getBrowsing() {
        return browsing;
    }

    public void setBrowsing(String browsing) {
        this.browsing = browsing;
    }

    public String getZwmc() {
        return zwmc;
    }

    public void setZwmc(String zwmc) {
        this.zwmc = zwmc;
    }

    public String getXbyq() {
        return xbyq;
    }

    public void setXbyq(String xbyq) {
        this.xbyq = xbyq;
    }

    public String getYxqk() {
        return yxqk;
    }

    public void setYxqk(String yxqk) {
        this.yxqk = yxqk;
    }

    public String getGzjy() {
        return gzjy;
    }

    public void setGzjy(String gzjy) {
        this.gzjy = gzjy;
    }

    public String getNlfw() {
        return nlfw;
    }

    public void setNlfw(String nlfw) {
        this.nlfw = nlfw;
    }

    public String getZdxl() {
        return zdxl;
    }

    public void setZdxl(String zdxl) {
        this.zdxl = zdxl;
    }

    public String getMzyq() {
        return mzyq;
    }

    public void setMzyq(String mzyq) {
        this.mzyq = mzyq;
    }

    public String getZprs() {
        return zprs;
    }

    public void setZprs(String zprs) {
        this.zprs = zprs;
    }

    public String getYxqz() {
        return yxqz;
    }

    public void setYxqz(String yxqz) {
        this.yxqz = yxqz;
    }

    public String getGzdd() {
        return gzdd;
    }

    public void setGzdd(String gzdd) {
        this.gzdd = gzdd;
    }

    public String getZwms() {
        return zwms;
    }

    public void setZwms(String zwms) {
        this.zwms = zwms;
    }

    public String getLxdh() {
        return lxdh;
    }

    public void setLxdh(String lxdh) {
        this.lxdh = lxdh;
    }

    public String getLxry() {
        return lxry;
    }

    public void setLxry(String lxry) {
        this.lxry = lxry;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public String getIsshowphone() {
        return isshowphone;
    }

    public void setIsshowphone(String isshowphone) {
        this.isshowphone = isshowphone;
    }

    public String getShoufei() {
        return shoufei;
    }

    public void setShoufei(String shoufei) {
        this.shoufei = shoufei;
    }

    public String getGzsj() {
        return gzsj;
    }

    public void setGzsj(String gzsj) {
        this.gzsj = gzsj;
    }

    public List<String> getTupian() {
        return tupian;
    }

    public void setTupian(List<String> tupian) {
        this.tupian = tupian;
    }

    public String getXinzidanwei() {
        return xinzidanwei;
    }

    public void setXinzidanwei(String xinzidanwei) {
        this.xinzidanwei = xinzidanwei;
    }
}
