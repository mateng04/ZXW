...
import com.mobile.zxw.myapplication.adapter.PopAdapter;
import com.mobile.zxw.myapplication.bean.PopItem;
import com.mobile.zxw.myapplication.until.PopupUtils;
...
import static com.mobile.zxw.myapplication.until.StaticUtils.FINISHED_FLAG;
import static com.mobile.zxw.myapplication.until.StaticUtils.ONLY_ONE_POP;
import static com.mobile.zxw.myapplication.until.StaticUtils.TRIPLE_POP;
...
public class PopupDownMenu {
    ...
    private Map<Integer, List<PopItem>> allMap = new TreeMap<>();
    ...
    public PopupDownMenu(Context mContext, final List<PopItem> itemList, int width, int height, View view, Drawable drawable, ListView... listViews) {
        ...
        //为popList中的数据初始化
        PopupUtils.popListInit(allMap, itemList);

        //初始化状态（若第二列存在元素就显示→显示后若第三行有也同理显示）
        try {
            List<PopItem> temp1 = allMap.get(allMap.get(0).get(0).getId());
            if (temp1 != null) {
                mList.addAll(temp1);
                List<PopItem> temp2 = allMap.get(temp1.get(0).getId());
                if (temp2 != null) {
                    mList2.addAll(temp2);
                }
            }
    ...
    private void initOnePop() {
        ...
        //Listview与适配器的初始化
        firstAdapter = new PopAdapter(mContext, allMap.get(0));
...
//                results[0] = popList.get(0).get(position).getContent();
                results[0] = allMap.get(0).get(position).getContent();
...
//                results[0] = itemList.get(position).getContent();
                ...
                try {
                    list2 = allMap.get(allMap.get(0).get(position).getId());
                } catch (IndexOutOfBoundsException e) {
                    e.printStackTrace();
                }
...
//                    }
//                }
                List<PopItem> poList = allMap.get(0);
                for(PopItem popItem:poList){
                    if(popItem.getId() == mList.get(position).getPid()){
                        results[0] = popItem.getContent();
                    }
                }
...
//                results[0] = popList.get(0).get(position).getContent();
                ...
                //如果有则替换并notify
                    ...
                    mList.addAll(list2);
                    mList2.clear();
                    if (allMap.get(list2.get(0).getId()) != null) {
                        mList2.addAll(allMap.get(list2.get(0).getId()));
                    }
                    secondAdapter.notifyDataSetChanged();
...
//                results[0] = itemList.get(mList.get(position).getPid()-1).getContent();
                ...
                try {
                    list3 = allMap.get(mList.get(position).getId());
                } catch (IndexOutOfBoundsException e) {
                    e.printStackTrace();
                }
