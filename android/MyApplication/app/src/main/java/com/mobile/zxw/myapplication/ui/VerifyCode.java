...
public class VerifyCode extends View {
    ...
    private void initData(){
        System.out.println("mCodeLength-------"+mCodeLength);

        System.out.println("isContainChar-------"+isContainChar);
...
//        UNSPECIFIED：表示子布局想要多大就多大，很少使用
    ...
    private Bitmap createBitmapValidate(){
        ...
        for (int i = 1; i <= length; i++) {
            ...
            //设置字体的绘制位置
            canvas.drawText(String.valueOf(mCodeText.charAt(i - 1)), (i - 1) * charLength+ 40,
                    mHeight * 3 / 5f, mPaint);
    ...
    public void refresh(){
        mCodeText= getValidationCode(mCodeLength,isContainChar);
        System.out.println("mCodeText-------"+mCodeText);
        bitmap = createBitmapValidate();
        invalidate();
    }
}