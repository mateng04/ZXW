...
//        mImageGridAdapter = new ImageGridAdapter(this, null);
        ...
        System.out.println("Build.VERSION_CODES.JELLY_BEAN ----------"+Build.VERSION_CODES.JELLY_BEAN );

        if (Build.VERSION.SDK_INT > Build.VERSION_CODES.JELLY_BEAN) {
            if (checkPermission(Manifest.permission.WRITE_EXTERNAL_STORAGE)) {
                System.out.println("Build.VERSION.SDK_INT --------111111111111111111--" );
                new ImageDataSource(this, null, this);
            } else {
                System.out.println("Build.VERSION.SDK_INT --------22222222222222--" );
                ActivityCompat.requestPermissions(this, new String[]{Manifest.permission.WRITE_EXTERNAL_STORAGE}, REQUEST_PERMISSION_STORAGE);
            }
        } else {
            System.out.println("Build.VERSION.SDK_INT --------3333333333333--" );
            new ImageDataSource(this, null, this);
        }
    }
...
//                    mImageGridAdapter.refreshData(imageFolder.images);
    ...
    public void onImagesLoaded(List<ImageFolder> imageFolders) {
        ...
        //新增的判断
        if(mImageFolders !=null && (imageFolders.size() == mImageFolders.size() )){
            return;
        }
...
//        mRecyclerAdapter.notifyItemChanged(position + (imagePicker.isShowCamera() ? 1 : 0));// 17/4/24  fix the position while click right bottom preview button
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        System.out.println("Build.VERSION.SDK_INT --------onActivityResult--" );
        if (data != null && data.getExtras() != null) {
            System.out.println("Build.VERSION.SDK_INT --------onActivityResult-1111111111111-" );
            if (resultCode == ImagePicker.RESULT_CODE_BACK) {
                System.out.println("Build.VERSION.SDK_INT --------onActivityResult-44444444444444-" );
